...
def example2feature(example, max_seq_length, tokenizer):
    ...
    base_tokens = ["[UNK]"] + ["[UNK]"] * len(ori_tokens) + ["[UNK]"] * 2
...
if __name__ == "__main__":
    ...
    allpos_kn = []
    max_igs = 0
    with open(os.path.join(allpos_dir, f"bert-{target_rel}-bag{target_bag}.json"), "r") as f:
        raw = json.load(f)
        max_igs = raw["max_igs"]
        for kn in raw["kn"]:
    ...
    for i, (eval_example, max_ig) in enumerate(zip(sentences, max_igs)):
        ...
        plt.colorbar(im)
        plt.savefig(os.path.join(allpos_dir, f"bert-{target_rel}-P{target_bag}-{i}.pdf"))
        plt.close()